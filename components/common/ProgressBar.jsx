            ...
            <div className='flex justify-between items-start relative min-w-[800px] md:min-w-full'>
                ...
                <div className='absolute top-4 left-0 w-full flex items-center justify-between pointer-events-none z-0 px-4'>
                    {steps.slice(0, -1).map((_, index) => (
                        <div key={index} className={`flex-1 h-[2px] 
                            ${index < currentStep ? 'bg-[#0B2C4F]' : 'bg-[#E4E7EC]'}
                        `}></div>
                    ))}
                </div>
                ...
                {steps.map((step, index) => (
                    <div key={index} className='flex flex-col items-center z-10 cursor-pointer group bg-transparent relative'>
                        {/* Circle */}
                        <div className={`w-8 h-8 p-0.5 rounded-full flex items-center justify-center bg-[#fafafa] transition-all duration-300 z-10
                            ${index === currentStep ? 'border-2 border-main' : 'border-2 border-[#E9EAEB]'}
                            ${index < currentStep ? 'bg-[#0B2C4F] border-[#0B2C4F]' : ''}
                        `}>
                            {index === currentStep && (
