...
function AllUser({ setSelectedUser }) {
  ...
  const getStatusStyle = (status) => {
    switch (status) {
      case "Incomplete":
        return "bg-[#E0E0E0] text-black border-transparent";
      case "Pending":
        return "bg-[#FFA500] text-white border-transparent";
      case "Verified":
        return "bg-[#2196F3] text-white border-transparent";
      case "Package Selected":
        return "bg-[#9C27B0] text-white border-transparent";
      case "Credentials Sent":
        return "bg-[#009688] text-white border-transparent";
      case "Active":
        return "bg-[#4CAF50] text-white border-transparent";
      case "Suspended":
        return "bg-[#F44336] text-white border-transparent";
      case "Canceled":
        return "bg-[#795548] text-white border-transparent";
      case "License Exp":
        return "bg-[#D32F2F] text-white border-transparent";
      case "Deactivate":
        return "bg-[#607D8B] text-white border-transparent";
      default:
  ...
  const columns = [
    {
      ...
      accessor: "idHash",
      render: (row) => (
        <span className="font-medium text-sm leading-5 tracking-[-0.006em] text-[#16151C]">
          {row.idHash}
        </span>
      ),
    },
    {
      ...
      render: (row) => (
        <div
          ...
          onClick={() => setSelectedUser("userdetail")}
        >
          <Image
            src={row.avatar}
            alt={row.name}
    ...
    },
    {
      ...
      accessor: "date",
      render: (row) => (
        <span className="text-[#404040] font-semibold">{row.date}</span>
      ),
    },
    {
      header: "Expiry Date",
      ...
      render: (row) => (
        <div className="flex flex-col gap-1 w-24">
          ...
          <div className="h-1.5 w-full bg-[#E4E7EC] rounded-full overflow-hidden">
            <div
              className="h-full bg-[#00359E] rounded-full"
              style={{ width: "60%" }}
            ></div>
          </div>
        </div>
      ...
      render: (row, index) => (
        <div className="w-[171px]">
          <Commondropdown
            ...
            iconClassName={
              row.status === "Incomplete" ? "" : "brightness-0 invert"
            }
          />
        </div>
      ),
      ...
      render: (row) => (
        <div className="flex items-center gap-2 ">
          ...
          <button className="w-8 h-8 p-2 flex items-center justify-center border border-[#D5D7DA] rounded-lg bg-white hover:bg-gray-50 cursor-pointer">
            <Image src={TableMailIcon} alt="mail" width={16} height={16} />
          </button>
          <button className="w-8 h-8 p-2 flex items-center justify-center border border-[#D5D7DA] rounded-lg bg-white hover:bg-gray-50 cursor-pointer">
            <Image
              src={TableInvoiceIcon}
              alt="invoice"
              width={16}
              height={16}
            />
  ...
  return (
    <div className="text-black">
      <div className="flex items-center max-sm:flex-col max-sm:items-start max-sm:gap-4 justify-between mt-7.5 ">
        <h1 className="text-lg md:text-xl font-semibold ">User List</h1>
        <div className="flex max-sm:justify-end items-center gap-2 max-sm:self-end flex-wrap">
          <div className="group flex items-center gap-2 border border-[#D5D7DA] p-3 px-4 rounded-lg cursor-pointer hover:bg-main hover:text-white transition-colors">
            <Image
              src={CustomizeIcon}
              alt="customize"
              width={20}
          ...
          <div className="group max-sm:self-end border border-[#D5D7DA] p-2.5 rounded-lg cursor-pointer outline-0 hover:bg-(--color-main) hover:border-(--color-main) hover:text-white transition-colors">
            <Image
              src={CopyIcon}
              alt="media"
              width={24}
