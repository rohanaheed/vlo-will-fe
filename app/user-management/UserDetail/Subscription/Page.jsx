  ...
  const columns = [
    {
      ...
      sortable: true,
      render: (row) => (
        <span className="text-[#404040] font-medium text-sm">{row.date}</span>
      ),
    },
    {
      header: "Expiry Date",
      ...
      render: (row) => (
        <div className="flex flex-col gap-1 w-24">
          <span className="text-[#404040] font-medium text-sm">
            {row.expiryDate}
          </span>
    ...
    },
    {
      ...
      render: (row) => (
        <div className="flex items-center justify-between gap-3">
          <span className="font-medium text-[#404040] text-sm w-28">
            {row.plan}
          </span>
          <div className="items-center border border-black/16 shadow rounded-md px-2 py-0.5 inline-flex gap-1.5">
            ...
            ></div>
            <span
              className={`text-xs font-medium ${row.planAction === "Upgrade" ? "text-text-4" : "text-[#344054]"}`}
            >
              {row.planAction}
            </span>
          </div>
      ...
      render: (row) => (
        <div className="flex items-center gap-3">
          ...
          <div className="items-center border border-black/16 shadow rounded-md px-2 py-0.5 inline-flex gap-1.5">
            ...
            <span className="text-xs font-medium text-text-4">Paid</span>
          </div>
        </div>
      ),
    },
    ...
    },
    {
      ...
      render: (row) => (
        <span
          className={`px-3 py-1 w-24 inline-block text-center rounded-full text-sm font-medium text-white ${row.status === "Active" ? "bg-[#17B26A]" : "bg-[#D92D20]"}`}
        >
          {row.status}
        </span>
      ),
    ...
    <>
      ...
      <div className="mb-2.5 mt-6">
        <p className="text-base mb-2.5 text-[#33383F]">
          Current Plan: <span className="">Single Document</span>
        </p>
        <div className="w-full bg-[#E4E7EC] rounded-full h-1.5">
          <div
            className="bg-(--color-main) h-1.5 rounded-full"
            style={{ width: "70%" }}
          ></div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 mb-6 text-sm">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="border flex items-center gap-1 border-black/16 text-text-4 text-xs px-2 py-0.5 rounded-md font-medium shadow">
      ...
      <div className="flex md:flex-row flex-col md:items-center gap-2 mb-4">
        ...
        <div className="group inline-block self-end border border-[#D5D7DA] p-2.75 rounded-lg cursor-pointer hover:bg-(--color-main) hover:text-white transition-colors">
          <Image
            src={copyIcon}
            alt="media"
            width={24}
