  ...
  const columns = [
    {
      ...
      sortable: true,
      render: (row) => (
        <span className="text-[#16151C] font-medium text-sm">
          {row.creditNoteId}
        </span>
      ),
    },
    ...
    },
    {
      ...
      sortable: true,
      render: (row) => (
        <span className="text-[#404040] font-medium text-sm">{row.reason}</span>
      ),
    },
    {
      header: "Issue Date",
    ...
    },
    {
      ...
      render: (row) => {
        const statusStyles = {
          ...
          "Partially Applied":
            "bg-[#14B8A6] text-white border-[#14B8A6] w-37.5",
        ...
        if (row.status === "Outstanding")
          statusStyles["Outstanding"] =
            "bg-[#FF692E] text-white border-[#FF692E] w-37.5";
        if (row.status === "Partially Applied")
          statusStyles["Partially Applied"] =
            "bg-[#00A9A4] text-white border-[#00A9A4] w-37.5";

        return (
          <span
            className={`px-4 py-1.5 inline-block text-center rounded-full text-xs font-medium border ${statusStyles[row.status] || "bg-gray-500 text-white"}`}
