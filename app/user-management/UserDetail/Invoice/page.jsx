...
function Page() {
  ...
  const columns = [
    ...
    },
    {
      ...
      sortable: true,
      render: (row) => (
        <span className="text-[#404040] font-medium text-sm">{row.plan}</span>
      ),
    },
    {
      header: "Date",
      ...
      sortable: true,
      render: (row) => (
        <span className="text-[#404040] text-sm font-medium">{row.date}</span>
      ),
    },
    {
    ...
    },
    {
      ...
      render: (row) => (
        <span
          className={`px-5 py-1.5 inline-block text-center rounded-full text-xs font-medium border ${
            row.status === "Active"
