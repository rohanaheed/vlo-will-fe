        ...
        {selectedUser === "usermanagement" && (
          <>
            ...
            <div className="border-b border-[var(--color-main)] mt-4 overflow-x-auto flex whitespace-nowrap">
              <button
                onClick={() => setIsTab("AllUser")}
                className={`${isTab === "AllUser" ? "bg-[var(--color-main)] text-white" : " text-[#828282]"} cursor-pointer font-medium p-2.5 rounded-t-[10px]`}
              >
                All User
              </button>
              <button
                onClick={() => setIsTab("90DaysRecycleBin")}
                className={`${isTab === "90DaysRecycleBin" ? "bg-[var(--color-main)] text-white" : "text-[#828282]"} cursor-pointer font-medium p-2.5 rounded-t-[10px]`}
              >
                90 Days Recycle Bin
              </button>
              <button
                onClick={() => setIsTab("Draft")}
                className={`${isTab === "Draft" ? "bg-[var(--color-main)] text-white" : "text-[#828282]"} cursor-pointer font-medium p-2.5 rounded-t-[10px]`}
              >
                Draft
              </button>
            </div>
            {isTab === "AllUser" && (
              <AllUser setSelectedUser={setSelectedUser} />
            )}
            {isTab === "90DaysRecycleBin" && (
              <NinetyDaysRecycleBin setSelectedUser={setSelectedUser} />
            )}
